...
# לעומת זאת עם כיבוי מסך בלבד מחזיק רק כ 12 שעות אבל אין מריחות מסך והשעון הפנימי ממשיך להיות מדוייק כל עוד לא מאפסים את המכשיר או שהסוללה נגמרת
# לכן יש בקוד כרגע את שתי האפשרויות והמשתנה הבא שולט האם הולכים לשינה עמוקה או לכיבוי מסך בלבד
DEEPSLEEP = False
######################################################################################################################
...
def toggle_power(pin):
    ...
    # הגדרה של דברים חשובים שצריכים להתבצע אם הכוח נהפך להיות פעיל בעקבות הלחיצה כלומר אם זו לחיצה שמדליקה
    # לאחר שינה עמוקה אין צורך בשורות הבאות כי בכל מקרה הקוד נקרא מהתחלה וזה גורם לכל הדברים הבאים בלי להגדיר אותם
...
def main_main():
    ...
    # אם הכוח לא פועל אז יש לכבות הכל
        ...
        if DEEPSLEEP:
            ...
            RD.value(0)
            LCD_POWER.value(0)
            # שולחים את המכשיר לשינה רגילה של 11.5 ימים !!!!!! אבל אם רוצים להעיר אז הלחיצה על כפתור גורמת לקוד להתחיל שוב ממיין כאילו ריסט
