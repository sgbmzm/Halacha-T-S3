...
while True:
    ...
    # אם הכוח לא פועל אז יש לכבות הכל
        ...
        '''
        # כרגע בוטל ההדפסה וההשהיה והכיבוי מתבצע מיד
        # הדפסה למסך
        tft.fill(0) # מחיקת המסך
        tft.write(FontHeb20,f'{reverse("שינה")}',30,75)
        tft.show() # כדי להציג את הנתונים על המסך
        time.sleep(1) # השהייה 5 שניות כדי שיהיה זמן לראות את ההודעה לפני שהמסך ייתמלא שוב בחישובים
        #tft.fill(0) # מחיקת המסך
        #tft.show() # כדי להציג את הנתונים על המסך
        '''
