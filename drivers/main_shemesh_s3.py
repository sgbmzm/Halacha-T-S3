...
VERSION = "24/03/2025:00"

####################################################################################################################
# משתנה מאוד חשוב ששולט על השאלה האם הכיבוי האוטמטי או כשלוחצים על כפתור הכיבוי יהיה למצב שינה עמוקה או רק לשינה רגילה
# שימו לב שהתעוררות משינה עמוקה גורמת מריחות שונות על המסך ולכן יש הבדלה בין המצבים הללו בקוד
# אסכם את כל המצב לגבי חיסכון בסוללה
# שינה עמוקה הכי חוסכת אבל כרגע גורמת למריחות על המסך לאחר ההתעוררות ממנה. כמו כן השעון הפנימי לא עדכני לאחר שינה עמוקה ולכן חייבים את השעון החיצוני צמוד
# לעומת זאת עם כיבוי מסך ושינה רגילה מחזיק רק ... שעות אבל אין מריחות מסך והשעון הפנימי ממשיך להיות מדוייק כל עוד לא מאפסים את המכשיר או שהסוללה נגמרת
# לכן יש בקוד כרגע את שתי האפשרויות והמשתנה הבא שולט האם הולכים לשינה עמוקה או לכיבוי מסך ושינה רגילה
DEEPSLEEP = True
######################################################################################################################
...
# אם המתח מעל 4.69 וולט והמשתמש רוצה בהירות מקס כפי שהוגדר בתחילת הקוד אז הבהירות הכי גבוהה
# אם המשתמש לא הגדיר בתחילת הקוד בהירות הכי גבוהה אז הבהירות תהיה בינונית אם המתח גדול מ 4.69 כלומר שלא מחובר רק לסוללה הפנימית
# בכל מקרה אחר מחובר רק לסוללה הפנימית או גם אם לחיצונית אבל היא חלשה הכוח הוא 255 שזה רבע בהירות, כדי לחסוך בחשמל
duty_for_backligth = PWM_MAX if (voltage >= max_battery_v and behirut_max) else 450 if voltage > max_battery_v else 255
...
def toggle_power(pin):
    ...
    # אם זו לחיצה שמדליקה כלומר שהכוח נהפך להיות פעיל מייבאים את קובץ מיין הראשי וזה מתחיל את התוכנה מהתחלה וכך מפעיל הכל
    # בשינה עמוקה אין בזה צורך כי בכל מקרה מתבצע ריסט ביציאה מהשינה העמוקה
    if power_state and not DEEPSLEEP:
        # אופצייה ראשונה שעובדת נפלא ברוך השם: קריאה לקובץ הראשי מיין שזה כאילו התחלה של הפעלה חדשה
        import main 
        # אפשרות שנייה שאולי צריך לבדוק: איפוס אמיתי. אבל אולי זה יותר יפגע בדיוק של השעון. לבדוק.
        ##### machine.reset()
...
def main_main():
    ...
    global power_state, last_voltage, start_time_for_automatic_deepsleep, start_time_for_check_and_set_time, is_bme280_connected, bme
    ...
    # כרגע מוגדר ל 120 שניות כי אחרת לא יוכלו לראות את כל ההסברים אם ייכבה קודם
    seconsd_to_start_auto_deepsleep = 120 
    ...
    '''
    if not DEEPSLEEP:       
        # תיקון כדי שחיבור לחשמל יפעיל את המסך. אבל זה עוזר רק אם המכשיר לא בשינה עמוקה וגם לא בשינה רגילה 
        # לכן כרגע אין לזה אפשרות לעבוד כי המכשיר בכל מקרה בשינה רגילה או עמוקה
        if last_voltage < max_battery_v and current_voltage > max_battery_v and abs(last_voltage - current_voltage) > 0.5:
            power_state = True
    '''       
    ...
    # יש שתי אופציות כיבוי והבחירה ביניהם מוגדרת בתחילת קובץ זה באמצעות המשתנה DEEPSLEEP
    # או שהולכים לשינה עמוקה או שהולכים לכיבוי רגיל שכולל בתוכה שינה רגילה לכמה ימים עד שיעירו את המכשיר באמצעות לחיצה על כפתור ההפעלה 
    else:

        # כולל טיפול מיוחד למסך אם נכנסים למצב שינה עמוקה כדי למזער נזקים של מריחות במסך שנגרמים בהתעוררות משינה עמוקה
        # אבל זה עדיין לא מספיק עוזר
        if DEEPSLEEP:
            ...
            all_pins_off() # חשוב ביותר כיבוי כל הפינים למעט פין 14 של כפתור ההתעוררות לפני הכניסה למצב שינה כדי למנוע בעיות מסך ובזבוז בטרייה
            
            # מצב שינה. היציאה ממצב שינה מתבצעת באמצעות לחיצה על הכפתור שמעיר את המכשיר וקורא שוב לקובץ מיין שקורא שוב לקובץ מיין שמש
            #seconds_sleep = 120 * 1000 # הערך הראשון הוא מספר השניות
            #machine.deepsleep(seconds_sleep) # מתעורר אחרי הזמן המוגדר כדי למנוע מריחות מסך
            machine.deepsleep() # לא מתעורר כל עוד שלא לוחצים על כפתור 14
            '''
            # אפשרויות נוספות שכרגע לא פעילות ורשומות רק לזיכרון שהן קיימות
            # machine.deepsleep() # עושה מריחות במסך ולכן אני מעיר אותו כל דקה
            # machine.lightsleep() # עושה מריחות כמו שינה עמוקה אבל אולי שומר את השעון הפנימי טוב יותר משינה עמוקה
            # machine.lightsleep(seconds_sleep) 
            '''
        # אם לא מוגדר שינה עמוקה הולכים לכיבוי מסך ולשינה רגילה    
        else:
            # כיבוי הכוח והתאורה וכל מה שקשור למסך
            BACKLIGHT.duty(PWM_MIN)
            RD.value(0)
            LCD_POWER.value(0)
            # שולחים את המכשיר לשינה רגילה של שנייה וזה מתחדש כל רגע ב וייל טרו הראשי
            # אבל אם רוצים להעיר אז הלחיצה על כפתור ההפעלה גורמת לקוד להתחיל שוב מקובץ מיין הראשי וזה כאילו ריסט
            time.sleep(1) # זה גורם לחסכון ממשי בצריכת החשמל כשלא הולכים לשינה עמוקה אלא רק לכיבוי מסך
            #time.sleep(999999) # שינה של 11.5 ימים לא חסכה בחשמל יותר משינה של שנייה אחת, אולי בגלל וייל טרו הראשי
