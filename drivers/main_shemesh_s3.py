...
# כברירת מחדל לא מעדכנים את הזמן של ds3231 משרת ntp
def sync_rtc_with_ds3231(update_ds3231_from_ntp = False):
...
locations = [
    ...
    {'heb_name': 'הקוטב-הצפוני', 'lat': 89.99, 'long': 0.0, 'altitude': 0.0, 'utc_offset': '', 'name': 'North Pole'} , # בעיה בספרייה גורמת שיש שגיאה אם מחשבים 90 מעלות. הבעיה לא קורית במחשב
...
sync_rtc_with_ds3231(update_ds3231_from_ntp = False)
...
def handle_button_press(specific_button):
    ...
    while specific_button.value() == 0:  # כל עוד הכפתור לחוץ
        if time.ticks_diff(time.ticks_ms(), start_time) > 3000:  # לחיצה ארוכה מעל 3 שניות ומתחת 6 שניות
            return "long"
...
def toggle_location(pin):
    ...
    # בלחיצה מאוד ארוכה 6 שניות יש לעדכן את הארטיסי הפנימי משרת אנטיפי
    elif duration == "long":
        sync_rtc_with_ds3231(update_ds3231_from_ntp = True)
        
    elif duration == "very_long":
...
while True:
    ...
    if power_state:
        ...
        voltage = read_battery_voltage()
        duty_for_backligth = 450 if voltage > 4.6 else 255
        ...
        main()
        current_screen = (current_screen + 0.2) % len(esberim)  # זה גורם מחזור של שניות לאיזה נתונים יוצגו במסך
